...
func localizeRenderedTUI(view string) string {
	...
	plain := []string{
		...
		"当前模式：快速开始 · 一句话直接开始写", "Chế độ hiện tại: Bắt đầu nhanh · Bắt đầu viết trực tiếp từ một câu yêu cầu",
		"当前模式：共创规划 · 先与 AI 对话澄清，再开始创作", "Chế độ hiện tại: Cùng lập kế hoạch · Trao đổi với AI để làm rõ trước khi bắt đầu sáng tác",
	...
	modelLabelStyle := lipgloss.NewStyle().Foreground(colorMuted).Width(12)
	for upstream, vietnamese := range vietnameseModelFieldLabels {
		view = strings.ReplaceAll(view, modelLabelStyle.Render(upstream+":"), modelLabelStyle.Render(vietnamese+":"))
	}

	accentBoldStyle := lipgloss.NewStyle().Foreground(colorAccent).Bold(true)
	for upstream, vietnamese := range vietnameseAccentTitles {
		view = strings.ReplaceAll(view, accentBoldStyle.Render(upstream), accentBoldStyle.Render(vietnamese))
	}

	mutedBoldStyle := lipgloss.NewStyle().Foreground(colorMuted).Bold(true)
	for upstream, vietnamese := range vietnameseMutedTitles {
		view = strings.ReplaceAll(view, mutedBoldStyle.Render(upstream), mutedBoldStyle.Render(vietnamese))
	}

	successStyle := lipgloss.NewStyle().Foreground(colorSuccess)
	for upstream, vietnamese := range vietnameseSuccessChrome {
		view = strings.ReplaceAll(view, successStyle.Render(upstream), successStyle.Render(vietnamese))
	}

	errorStyle := lipgloss.NewStyle().Foreground(colorError)
	for upstream, vietnamese := range vietnameseErrorChrome {
		view = strings.ReplaceAll(view, errorStyle.Render(upstream), errorStyle.Render(vietnamese))
	}

	dimStyle := lipgloss.NewStyle().Foreground(colorDim)
	view = strings.ReplaceAll(view, dimStyle.Render(" 省"), dimStyle.Render(" tiết kiệm "))

	startupInactive := lipgloss.NewStyle().Padding(0, 1).Foreground(colorMuted)
	startupActive := lipgloss.NewStyle().Padding(0, 1).Foreground(lipgloss.Color("#1c1a14")).Background(colorAccent).Bold(true)
	for upstream, vietnamese := range vietnameseStartupPills {
		view = strings.ReplaceAll(view, startupInactive.Render(upstream), startupInactive.Render(vietnamese))
		view = strings.ReplaceAll(view, startupActive.Render(upstream), startupActive.Render(vietnamese))
	}
	return view
}

var vietnameseStartupPills = map[string]string{
	"快速开始": "Bắt đầu nhanh",
	"共创规划": "Cùng lập kế hoạch",
}

var vietnameseAccentTitles = map[string]string{
...
var vietnameseMutedTitles = map[string]string{
	"仿写画像": "Hồ sơ mô phỏng phong cách",
}

var vietnameseSuccessChrome = map[string]string{
	"未发现问题": "Không phát hiện vấn đề",
}

var vietnameseErrorChrome = map[string]string{
	"导入失败":     "Nhập thất bại",
	"仿写画像处理失败": "Xử lý hồ sơ mô phỏng phong cách thất bại",
}

var vietnameseModelFieldLabels = map[string]string{
	"角色":   "Vai trò",
	"模型":   "Model",
	"推理强度": "Mức suy luận",
}
