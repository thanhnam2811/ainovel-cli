...
// bundle. It is deliberately additive: when a translation is missing, the
// upstream asset remains active and receives only a language-output directive.
// This lets upstream add or change prompts without breaking the Vietnamese fork.
func ApplyLocale(b *Bundle, locale string) error {
	if b == nil {
...
func applyVietnameseLocale(b *Bundle) {
	...
	b.Prompts.ArchitectShort = localizedCorePrompt("vi", "architect-short.md", "architect", b.Prompts.ArchitectShort)
	b.Prompts.ArchitectLong = localizedCorePrompt("vi", "architect-long.md", "architect", b.Prompts.ArchitectLong)
	b.Prompts.Writer = localizedCorePrompt("vi", "writer.md", "writer", b.Prompts.Writer)
	b.Prompts.Editor = localizedCorePrompt("vi", "editor.md", "editor", b.Prompts.Editor)
	...
	if raw, ok := readLocale("vi", "references/anti-ai-tone.md"); ok {
		b.References.AntiAITone = raw
	}
	if raw, ok := readLocale("vi", "styles/default.md"); ok {
		if b.Styles == nil {
			b.Styles = make(map[string]string)
		}
		b.Styles["default"] = raw
	}
}

func localizedCorePrompt(locale, name, role, fallback string) string {
	if raw, ok := readLocale(locale, "prompts/"+name); ok {
		...
		return appendDirective(raw+"\n\n"+strings.ReplaceAll(viSimulationGuidance, "{{role}}", role), viLanguageDirective)
	}
	return appendDirective(fallback, viLanguageDirective)
}
